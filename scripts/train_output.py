...
if __name__ == '__main__':
    ...
    # 数据集加载
    train_dataset = datasets.TensorDataset(cfg["train"], cfg["width"], cfg["height"], imgaug = True)
    val_dataset = datasets.TensorDataset(cfg["val"], cfg["width"], cfg["height"], imgaug = False)
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for imgs, targets in pbar:
            ...
            print("test:", iou_loss.item())
            loss_data.append(iou_loss.item())
        ...
        # 模型保存
        if (iou_loss < 0.15 and epoch > 0) or (epoch < (cfg["epochs"] - 30) and epoch > 0):
            model.eval()
            #模型评估
            print("computer mAP...")
            _, _, AP, _ = utils.evaluation(val_dataloader, cfg, model, device)
            print("computer PR...")
            precision, recall, _, f1 = utils.evaluation(val_dataloader, cfg, model, device, 0.3)
            # print("Precision:%f Recall:%f AP:%f F1:%f"%(precision, recall, AP, f1))

            torch.save(model.state_dict(), "weights/%s-%d-epoch-%fap-model.pth" %
                      (iou_loss.item(), epoch, AP))
